...
class Pupil(object):
    ...
    def loopContours(self):
        ...
        for cnt in self.contours:
            ...
            # frame, contour, value, color, thickness
                if 3 <len(approx)<10:
                    self.area=area
...
def redrawAll(app,canvas):
    ...
    if app.currentlyCalibrating:
        canvas.create_text(app.width//2, 230, text="How to Calibrate!",font='Arial 15 bold')
    ...
    canvas.create_text(app.width//2,start+3*spacing,text="Step 2: Press '1' to be able to move your mouse")
    canvas.create_text(app.width//2,start+4*spacing,text="Step 2: Press '2' to be able to click and scroll around")
    #current modes
    canvas.create_rectangle(app.width//2-120,start+6*spacing-10,app.width//2+120, start+8*spacing+10)
    canvas.create_text(app.width//2,start+6*spacing,text=f"Calibration Mode: %s" %("On" if app.currentlyCalibrating else "Off"),font='Arial 12 bold',fill=f"%s" %("green" if app.currentlyCalibrating else "black"))
    canvas.create_text(app.width//2,start+7*spacing,text=f"Clicking and Scrolling Mode: %s" %("On" if app.clickscrollingFeature else "Off"),font='Arial 12 bold',fill=f"%s" %("green" if app.clickscrollingFeature else "black"))
    canvas.create_text(app.width//2,start+8*spacing,text=f"Mouse Moving Mode: %s" %("On" if app.movingEyesFeature else "Off"),font='Arial 12 bold',fill=f"%s" %("green" if app.movingEyesFeature else "black"))
...
def keyPressed(app,event):
    ...
    #moving mouse clicking and scrolling
    if event.key=="2": 
    ...
    #to toggle between calibrating and not calibrating
    if event.key=="d":
